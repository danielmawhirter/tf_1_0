...
Status RunBenchmark(int num_runs, const std::vector<InputLayerInfo>& inputs,
  ...
  for (const InputLayerInfo& input : inputs) {
    ...
    switch (input.data_type) {
      case DT_INT32: {
        ...
        int_tensor = int_tensor.constant(rand()%1000);
        break;
      }
      case DT_FLOAT: {
        auto float_tensor = input_tensor.flat<float>();
        float_tensor = float_tensor.constant(float(rand()%1000)/1000.);
        break;
      }
      case DT_QUINT8: {
  ...
  std::cerr << "The execution took " << std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() << std::endl;
...
//   for (int i = 0; i < num_runs; ++i) {
    Status run_status = RunBenchmark(num_runs, inputs, outputs, session, stats);
    if (!run_status.ok()) {
//       LOG(INFO) << "Failed on run " << i;
      LOG(INFO) << "Failed on run ";
      return run_status;
    }
...
int Main(int argc, char** argv) {
  ...
  std::vector<Flag> flag_list = {
      ...
      Flag("inter_op_threads", &inter_op_threads, "number of inter op threads"),
      Flag("intra_op_threads", &intra_op_threads, "number of intra op threads"),
  ...
  LOG(INFO) << "Inter op threads: [" << inter_op_threads<< "]";
  LOG(INFO) << "Intra op threads: [" << intra_op_threads<< "]";
  ...
  Status initialize_status =
      InitializeSession(inter_op_threads, intra_op_threads, graph, &session, &stats);
