...
// efficient.
template <typename T>
bool ExplicitVectorMatrixOptimizationOrg(
...
template <>
bool ExplicitVectorMatrixOptimizationOrg<Eigen::half>(
    const Tensor& a, const Tensor& b,
    const Eigen::array<Eigen::IndexPair<Eigen::DenseIndex>, 1>& dim_pair,
    Tensor* out) {
  return false;
...
struct LaunchMatMulBase {
  static void launch(
    ...
    bool was_vector = ExplicitVectorMatrixOptimizationOrg<T>(a, b, dim_pair, out);
    if (!was_vector) {
#endif  // TENSORFLOW_USE_SYCL
...
template <typename Device, typename T, bool USE_CUBLAS>
class MatMulOrgOp : public OpKernel {
 public:
  explicit MatMulOrgOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
...
#define REGISTER_CPU(T)                                                        \
  REGISTER_KERNEL_BUILDER(                                                     \
      Name("MatMulOrg").Device(DEVICE_CPU).TypeConstraint<T>("T"),                \
      MatMulOrgOp<CPUDevice, T, false /* cublas, ignored for CPU */>);            \
  REGISTER_KERNEL_BUILDER(                                                     \
      Name("MatMulOrg").Device(DEVICE_CPU).TypeConstraint<T>("T").Label("eigen"), \
      MatMulOrgOp<CPUDevice, T, false /* cublas, ignored for CPU */>)

#define REGISTER_GPU(T)                                            \
  REGISTER_KERNEL_BUILDER(                                         \
      Name("MatMulOrg").Device(DEVICE_GPU).TypeConstraint<T>("T"),    \
      MatMulOrgOp<GPUDevice, T, true /* cublas, true by default */>); \
  REGISTER_KERNEL_BUILDER(Name("MatMulOrg")                           \
                              .Device(DEVICE_GPU)                  \
                              .TypeConstraint<T>("T")              \
                              .Label("cublas"),                    \
                          MatMulOrgOp<GPUDevice, T, true /* cublas */>)
...
#define REGISTER_SYCL(T)                                         \
  REGISTER_KERNEL_BUILDER(                                       \
      Name("MatMulOrg").Device(DEVICE_SYCL).TypeConstraint<T>("T"), \
      MatMulOrgOp<SYCLDevice, T, false /* xxblas */>);              \
  REGISTER_KERNEL_BUILDER(Name("MatMulOrg")                         \
                              .Device(DEVICE_SYCL)               \
                              .TypeConstraint<T>("T")            \
                              .Label("eigen"),                   \
